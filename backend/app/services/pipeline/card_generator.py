...
반드시 아래 JSON 형식으로만 출력하세요:
{
  ...
  "answer": "2~3문장. 고객 질문에 직접 답하되 [문서]의 구체적 수치·기간·조건·절차를 반드시 포함."
}

answer 작성 원칙:
1. **고객 질문에 직접 답하는 문장으로 시작** (질문을 피하거나 돌려 말하지 말 것)
2. **[문서]의 구체적 정보를 그대로 반영** (생략 금지)
   - 기간: "5일", "2주", "6개월" 등 숫자 명시
   - 조건: "~인 경우에만", "~일 때는" 등 조건 명시
   - 절차: 단계가 있으면 순서대로 기술
   - 수치: %, 회, 배 등 그대로
3. **예시**
   - 격리 기간 문의 → "결핵 확진 환자는 전염성이 소실될 때까지 격리하며, 객담 도말검사에서 2회 연속 음성이 확인되면 격리를 해제합니다."
   - 신고 기준 문의 → "유행성이하선염 의심환자는 확진 여부와 관계없이 24시간 이내에 관할 보건소에 신고해야 합니다."
   - 치료 문의 → "C형간염 치료율은 유전자형에 따라 다르며, 효과적인 치료제 도입으로 70~90% 치료율을 보입니다."

규칙:
- answer는 반드시 [문서]에 명시된 내용에만 근거할 것
- [문서]에 없는 수치·날짜·기관명은 절대 생성하지 말 것
- [문서]에 명확한 답이 없으면: "관련 지침에서 명확한 내용을 찾지 못했습니다."
- 말투: 상담사가 고객에게 직접 말하는 구어체 ('~입니다', '~하시면 됩니다')
- 지침 원문을 그대로 붙여넣지 말고 핵심만 쉽게 재서술할 것
- '병원 방문 권유', '진료 받으세요', '전문의 상담' 등 일반 의학 조언 추가 금지
- 절대 금지 문구: "질병관리청에 문의", "관련 기관에 문의", "자세한 사항은 ~에 문의"
  → 해당 문장 통째로 삭제하고 핵심 답변만 남길 것"""
