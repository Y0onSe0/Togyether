...
- 원문 의미를 최대한 유지하면서 최소한의 정제만 수행
  ...
  (2) 병명만 공식 병명으로 정규화
  (3) **병명은 대화 전체에서 반드시 유지** — 현재 발화에 병명이 없어도
      앞 대화에서 언급된 병명을 query와 disease_name에 반드시 포함
      예) 앞서 "씨형 간염" 언급 후 고객이 "신고를 해야 되는지요?" 라고 하면
          query: "C형간염 신고 여부 문의" (병명 반드시 포함)
          disease_name: "C형간염"
...
- 현재 발화 또는 앞 대화에서 언급된 특정 **사람** 감염병명을 공식 병명으로 정규화
- 대화 중 병명이 한 번이라도 언급됐으면 이후 발화에서도 반드시 유지, 없어지면 안 됨
- 병명이 전혀 언급되지 않은 경우만 null
